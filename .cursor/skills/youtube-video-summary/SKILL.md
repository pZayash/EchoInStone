...
- [ ] 8. При follow-up — читать тот же CSV/JSON, не гонять echoinstone заново
```
...
1. …
2. …
(4–6 пунктов — см. правила ниже)

**Транскрипт:** `results/…/speaker_transcriptions.csv` — можно задавать вопросы по содержанию.
```

### Вопросы для углубления (обязательно)

В **конце** ответа в чате — блок в стиле [NotebookLM](https://notebooklm.google/) suggested questions: **4–6** коротких вопросов, которые пользователь может задать агенту дальше. Язык = язык сообщения пользователя.

**Правила:**

- Каждый вопрос **отвечаем по транскрипту** — без домыслов и внешних фактов.
- Формулировки **конкретные к этому ролику** (имена, кейсы, цифры, спорные тезисы), не шаблоны вроде «о чём видео?».
- Смешивать типы (минимум по одному, где уместно):
  - **уточнение** термина или тезиса спикера;
  - **сравнение** (старый vs новый подход, два кейса);
  - **практика** («как применить к …», «что сделать в 1С/проекте»);
  - **критика / границы** (ограничения, Q&A, где спикер сам оговорился);
  - **деталь по таймкоду** — при необходимости `[HH:MM:SS]` в скобках.
- Нумерованный список; одна строка — один вопрос; без пояснений под пунктами.
- Не дублировать буллеты из «О чём» и «Ключевые тезисы» — вопросы **углубляют**, а не пересказывают.

**Пример формата (смысл, не копировать дословно):**

1. Чем цепочка «гипотеза → эксперимент → данные» отличается от согласованного ТЗ в кейсе с отчётом по остаткам?
2. Почему спикер считает, что логи 1С недостаточны, и что он предлагает вместо этого? [20:56]
3. В каких условиях data-driven подход, по его словам, неуместен — и что остаётся от классической оценки?
4. Как формулировать гипотезу, если заказчик просит «ещё одну кнопку»?
5. Что изменилось в WMS после доработки алгоритма и какая метрика это подтверждает?

Если пишется `summary.md` ([summarizev2](../../commands/summarizev2.md)) — тот же блок **в конце файла** (после «Следующие шаги» / Q&A), заголовок `## Вопросы для углубления`.

## 4. Вопросы позже

Читать тот же `speaker_transcriptions.csv` (или `.json`) в `results/…`. Повторный `echoinstone` — только по запросу «обнови транскрипт».

Если пользователь **кликает / копирует** один из предложенных вопросов из §3 — отвечать по транскрипту, с таймкодами где уместно; не перегенерировать резюме целиком.
